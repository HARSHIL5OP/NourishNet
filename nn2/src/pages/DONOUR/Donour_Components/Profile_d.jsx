...
const Profile_d = () => {
    ...
    return (
        ...
        <div 
            className={`fixed top-0 right-0 h-full w-80 bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 z-50 border-4 ${showProfile ? 'translate-x-0' : 'translate-x-full'}`}
        >
            {/* Profile Header */}
            <div className="bg-gradient-to-r from-blue-500 to-teal-400 p-6 text-white relative">
                ...
                <div className="flex items-center">
                    <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-white">
                        <img 
                            src={userData?.OrganizationProfileImage || "https://placehold.co/100x100"} // Use profile picture from user data or default placeholder
                            alt="Profile"
                            className="w-full h-full object-cover"
                        />
                    </div>
                    <div className="ml-4">
                        <h1 className="text-xl font-bold">{userData?.Organization_name || "Donor Name"}</h1>
